...
- Public surface: `decode`, `pack_pcm`, `Decoder`, `decode_frame`,
  `StreamHeader` / `StreamInfo`, `FrameDescriptor`, and the crate's
  `Error` / `Result` types.
...
- Crate-internal test-only encoder (`#[cfg(test)] mod encoder`) that
  manufactures self-consistent TTA1 streams for roundtrip testing,
  since no reference TTA fixtures are checked in.
